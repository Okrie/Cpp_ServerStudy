...
bool IOCompletionPort::Initialize()
{
    ...
    {
        printf_s("[ERROR] winsock 소켓 생성 실패\n");
        return false;
    }
...
void IOCompletionPort::WorkerThread()
{
    ...
    while(m_bWorkerThread)
    {
        ...
        bResult = GetQueuedCompletionStatus(m_hIOCP,
			...
			(PULONG_PTR)&pCompletionKey,	// completion key       ??error? lpdword -> pulong_Ptr
        ...
        if(!bResult && recvBytes == 0)
        {
            ...
            printf_s("[INFO] 메세지 송신 - Bytes : [%d], Msg : [%s]\n", pSocketInfo->dataBuf.len, pSocketInfo->dataBuf.buf);
            ...
            // 클라이언트로 다시 응답을 받기 위해 WSARecv 호출
            nResult = WSARecv(
                pSocketInfo->socket,
                &(pSocketInfo->dataBuf),
                1,
